    ...
    <section class="center-panel" style="flex:1; display:flex; flex-direction:column;">
      <div class="map-top" style="margin-bottom:8px;" v-show="currentPage === 'map'">
        <div class="btn-group" style="margin-left: auto;">
          ...
          <!-- 当没有生成图片时显示选择按钮 -->
          <button class="btn map-control-btn" v-if="!selectMode && !hasGeneratedImage" @click="enterSelectMode">选择</button>
...
function redraw() {
  ...
  if (selectState.hasSelection && selectState.geoPoints && selectState.geoPoints.length > 0) {
    ...
    for (const geoPoint of selectState.geoPoints) {
      // 验证地理坐标点
      if (!geoPoint || isNaN(geoPoint.lng) || isNaN(geoPoint.lat)) {
        console.warn('跳过无效的地理坐标点:', geoPoint);
        continue;
      }
      
      const screenPoint = lngLatToContainerPixel(geoPoint);
      if (screenPoint && !isNaN(screenPoint.x) && !isNaN(screenPoint.y)) {
        const adjustedPoint = adjustPixelForRedraw(screenPoint);
        if (adjustedPoint && !isNaN(adjustedPoint.x) && !isNaN(adjustedPoint.y)) {
          screenPoints.push(adjustedPoint);
          validPointsCount++;
        }
      }
    }
    
    console.log(`重绘: 总地理点数 ${selectState.geoPoints.length}, 有效屏幕点数 ${validPointsCount}`);
  ...
  if (selectState.drawing && selectState.currentPath && selectState.currentPath.length > 1) {
    // 验证路径中的点
    const validPath = selectState.currentPath.filter(point => 
...
function adjustPixelForRedraw(pixel) {
  if (!pixel || isNaN(pixel.x) || isNaN(pixel.y)) {
    console.error('adjustPixelForRedraw: 无效的像素坐标', pixel);
    return { x: 0, y: 0 };
  }
  ...
  try {
    ...
    // 根据地图倾斜角度进行补偿
    const validPitch = typeof pitch === 'number' && !isNaN(pitch) ? pitch : 0;
    const pitchFactor = validPitch / 90; 
    const adjustedDy = dy * (1 + pitchFactor * 0.15);
    
    // 根据旋转角度进行补偿
    const validRotation = typeof rotation === 'number' && !isNaN(rotation) ? rotation : 0;
    const rotationRad = validRotation * Math.PI / 180;
    ...
    if (isNaN(result.x) || isNaN(result.y)) {
      console.error('重绘调整后的像素坐标包含NaN:', result);
      return pixel; // 返回原始值作为fallback
    }
    
    return result;
  } catch (error) {
    console.error('重绘像素调整错误:', error);
    return pixel; // 错误时返回原始值
  }
}

function saveGeoPointsFromScreen(points) {
  if (!points || !Array.isArray(points)) {
    console.error('saveGeoPointsFromScreen: points参数无效', points);
    selectState.geoPoints = null;
    return;
  }
  ...
  console.log(`坐标转换结果: 总点数 ${points.length}, 有效点数 ${validPointsCount}`);
  
  // 只有至少3个点才能形成多边形
  selectState.geoPoints = geoPoints.length >= 3 ? geoPoints : null;
  selectState.hasSelection = geoPoints.length >= 3;
}
...
function containerPixelToLngLat(pixel) {
  ...
  // 增强输入验证
  if (!map || !AMap || !pixel) {
    console.error('containerPixelToLngLat: 参数无效', { map: !!map, AMap: !!AMap, pixel });
    return null;
  }
  ...
  if (typeof pixel.x !== 'number' || typeof pixel.y !== 'number' || 
    ...
    console.error('containerPixelToLngLat: 像素坐标无效', pixel);
    return null;
  }
  ...
  try {
    ...
    // 使用更安全的调整函数
    const adjustedPixel = adjustPixelForMapStateSafe(pixel, pitch, rotation);
    
    // 验证调整后的坐标
    if (!adjustedPixel || isNaN(adjustedPixel.x) || isNaN(adjustedPixel.y)) {
      console.error('containerPixelToLngLat: 调整后的坐标无效', adjustedPixel);
      return null;
    }
    ...
    if (!lnglat) {
      console.error('containerPixelToLngLat: 坐标转换返回空值');
      return null;
    }
    
    const lng = parseFloat(lnglat.getLng());
    const lat = parseFloat(lnglat.getLat());
    
    // 严格的经纬度验证
    if (isNaN(lng) || isNaN(lat) || !isFinite(lng) || !isFinite(lat)) {
      console.error('containerPixelToLngLat: 转换后的经纬度无效', { lng, lat });
      return null;
    }
    
    // 验证经纬度范围
    if (lng < -180 || lng > 180 || lat < -90 || lat > 90) {
      console.error('containerPixelToLngLat: 经纬度范围无效', { lng, lat });
      return null;
    }
    ...
    console.error('containerPixelToLngLat: 坐标转换错误', e);
    return null;
  }
}

function adjustPixelForMapStateSafe(pixel, pitch, rotation) {
  // 严格的输入验证
  if (!pixel || typeof pixel.x !== 'number' || typeof pixel.y !== 'number') {
    console.error('adjustPixelForMapStateSafe: 无效的像素坐标', pixel);
    return { x: 0, y: 0 };
  }
  
  if (isNaN(pixel.x) || isNaN(pixel.y) || !isFinite(pixel.x) || !isFinite(pixel.y)) {
    console.error('adjustPixelForMapStateSafe: 像素坐标包含NaN或无限值', pixel);
    return { x: 0, y: 0 };
  }
  
  const canvas = overlayCanvas.value;
  if (!canvas) {
    console.warn('adjustPixelForMapStateSafe: overlayCanvas未找到，使用默认中心');
    return pixel;
  }
  ...
  try {
    ...
    if (isNaN(centerX) || isNaN(centerY)) {
      console.error('adjustPixelForMapStateSafe: 中心点坐标无效', { centerX, centerY });
      return pixel;
    }
    ...
    // 使用安全的数学运算
    const validPitch = typeof pitch === 'number' && !isNaN(pitch) ? Math.max(-90, Math.min(90, pitch)) : 0;
    const pitchFactor = Math.max(-1, Math.min(1, validPitch / 60));
    ...
    if (isNaN(result.x) || isNaN(result.y) || !isFinite(result.x) || !isFinite(result.y)) {
      console.error('adjustPixelForMapStateSafe: 计算结果无效', result);
      return pixel;
    }
    ...
    console.error('adjustPixelForMapStateSafe: 计算过程中发生错误', error);
    return pixel;
  }
}
...
async function analyzeWithAI() {
  ...
  // 如果已有街景图分析结果，添加到提示中
  if (streetViewAnalysisResults.value && streetViewAnalysisResults.value.length > 0) {
    promptText += `\n\n检测到已有 ${streetViewAnalysisResults.value.length} 个街景图分析结果，将一并用于AI分析。`;
  }
