...
const contourState = reactive({
  isPreviewing: false,           
  isProcessing: false,           
  isShowingContours: false,      
  statusText: '点击"预览区域"开始选择基地轮廓',
  captureRect: null,             
  captureMapState: null,       
  allPolygons: [],               
...
function initMap(AMap) {
  ...
  redrawOn.forEach(ev => {
    try { 
      map.on(ev, () => {
        setTimeout(redrawBasePolygons, 100); 
      }); 
    } catch(e) { 
...
function confirmPurpleContours() {
  if (contourState.selectedIndices.size === 0) {
    contourState.statusText = '请先选择轮廓';
    return;
  }
...
async function finishSelectModeAndSave() {
  ...
  try {
    ...
    // 点击卫星图按钮，保存 big 和 small
    await clickSatelliteLayer();
    await new Promise(resolve => setTimeout(resolve, 1500)); 
    await saveBigAndSmallScreenshots();  
    
    // 点击标准图层按钮，保存 standard_base
    await clickStandardLayer();
    await new Promise(resolve => setTimeout(resolve, 1500)); 
    await saveStandardBaseScreenshot();  
    
    // 设置 pitch 为 0，保存 big_over 和 small_over
